...
\paragraph{Hypothèse :} on suppose donc $\overline{\beta}=a_{\max}$ afin d'avoir le caractère irréductible du semi-groupe.
...
\begin{itemize}
    ...
    \item Déterminons $x^*$.
    \begin{eqnarray*}
        (A^*x^*)(a) = \alpha^*x^*(a) &\Longrightarrow& -\mu(a)x^*(a)+(x^*)'(a)+\beta(a)v(0)=\alpha^*x^*(a) \\
        &\Longrightarrow& (x^*)'(a) = \alpha^*x^*(a) - \beta(a)x^*(0)+\mu(a)x^*(a) \\
        &\Longrightarrow& x^*(a) = x^*(0)\frac{\ex^{\alpha^*a}}{\Pi(a)} - x^*(0)\int_0^a\frac{\ex^{\alpha^*a}}{\Pi(a)}\ex^{-\alpha^* s}\Pi(s)\beta(s)\d s \\
        &\Longrightarrow& x^*(a) = x^*(0)\frac{\ex^{\alpha^*a}}{\Pi(a)}\Big(1-\int_0^a\ex^{-\alpha^*s}K(s)\d s\Big)
    \end{eqnarray*}
\end{itemize}
Pour satisfaire la condition $\langle x,x^*\rangle =1$, on peut par exemple prendre $x(0)=1$ et déterminer $x^*(0)$ tel que 
\begin{eqnarray*}
    \langle x,x^*\rangle = 1 &\iff& \int_0^{a_{\max}}x(a)x^*(a)\d a \\
    &\iff& x^*(0)\int_0^{a_{\max}} \Big(1-\int_0^a\ex^{-\alpha^*s}K(s)\d s\Big) \d a = 1 \\
    &\iff& x^*(0) = \frac{1}{a_{\max}-\displaystyle{\int_0^{a_{\max}}}\displaystyle{\int_0^a}\ex^{-\alpha^* s}K(s)\d s\d a}
\end{eqnarray*}
