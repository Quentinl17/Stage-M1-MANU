...
\begin{frame}
    \begin{itemize}
        ...
        \end{equation*} \pause
        \item On peut montrer que le modèle de Verhulst admet deux points d'équilibres : $0$ et $K$. Et, $K$ est localement asymptotiquement stable.
    \end{itemize}
\end{frame}
...
\begin{frame}
    \begin{itemize}
        \item On va considérer l'équation 
        \begin{equation*}
        ...
        \begin{equation*}
            ...
            \end{array}\right.
        \end{equation*} \pause
        \item Comme dans la section précédente, on considère $x: \bbR_+ \rightarrow L^1(0,a_{\max})$ et l'opérateur $A$.
        \begin{equation*}
            \left\{\begin{array}{l}
            ...
            \end{array}\right.
        \end{equation*}\pause
        \item On pose $f : L^1(0,a_{\max}) \rightarrow L^1(0,a_{\max}), \ u \mapsto - \gamma \|u\|_{L^1}u$.
    \end{itemize}
\end{frame}
...
\begin{frame}
    \begin{itemize}
        \item Déterminer les points d'équilibres revient à chercher une solution $x_0 \in D(A)$ de notre EDP qui ne dépend pas du temps. Donc résoudre $Ax_0+f(x_0) = 0$.\pause
...
\begin{frame}
    \begin{itemize}
        \item On rappelle que, dans l'étude asymptotique du modèle linéaire, on a obtenu
        \begin{align*}
        ...
        \item Si $R=1$, alors $0$ est le seul point d'équilibre. On a $\alpha^*=0$ et $\hat{A}=A$, donc $s(\hat{A})=s(A) =0$. On ne peut donc pas conclure sur la stabilité du point d'équilibre.
        \item Si $R>1$, alors on a deux points d'équilibre : $0$ et $x_0$. De plus, $\alpha^*>0$. Pour $0$, $\hat{A}=A$, donc $s(\hat{A})=s(A) >0$ par la proposition, donc $0$ est instable. \pause Pour $x_0$, $\hat{A}=A+f'(x_0)$ et
        \begin{itemize}
            \item[$\triangleright$] Si $0 < \lambda^* \leq \alpha^*$ alors $x_0$ est instable.
            \item[$\triangleright$] Si $\lambda^*=0$ alors on ne peut pas conclure sur la stabilité de $x_0$.
            \item[$\triangleright$] Si $\lambda^* < 0$ alors $x_0$ est localement exponentiellement asymptotiquement stable.
        \end{itemize}
    \end{itemize}
\end{frame}